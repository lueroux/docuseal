...
RUN sed -i 's/\r//' ./bin/shakapacker && chmod +x ./bin/shakapacker && echo "gem 'shakapacker'" > Gemfile && ./bin/shakapacker
...
RUN apk add --no-cache libpq vips redis onnxruntime leptonica chromium nss freetype fontconfig && \
